...
def resolve_whisper_repo(model_size: str) -> str:
    """Mapeia os 5 modelos suportados para seus repositórios no Hugging Face (Sal0 Karaoke v4.2.1)."""
    mapping = {
...
def is_model_downloaded(model_size: str) -> bool:
    """Verifica nos diretórios locais se um dos 5 modelos Whisper v4.2.1 já foi baixado."""
...
# Sistema de Logs de Diagnóstico v4.2.1
    ...
    if processing_lock.locked():
        with state_lock:
            if state.get("status") in ["idle", "error", "done"]:
                try:
                    processing_lock.release()
                    logger.info("Failsafe v4.2.1: Lock de concorrência obsoleto liberado com sucesso.")
                except Exception:
                    pass
            else:
                raise HTTPException(
