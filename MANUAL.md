...
No PC, os resultados são apresentados em uma grade compacta; no celular, os cartões se ajustam à largura disponível. Para remover vários arquivos, toque em **Selecionar** na categoria desejada, marque os itens e use **Excluir selecionados**. **Selecionar todos** marca todos os arquivos listados daquela categoria; confirme a exclusão para removê-los. O administrador pode selecionar arquivos de vários perfis. A exclusão é permanente e não afeta as outras categorias. Se alguma remoção falhar, os itens restantes continuam selecionados para nova tentativa. Use **Concluir seleção** para sair sem excluir.
