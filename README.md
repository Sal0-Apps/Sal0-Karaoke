...
O sistema inclui contas locais, isolamento de dados entre usuários, permissões administrativas, Biblioteca de mídias, perfis de renderização, revisão opcional de legendas, notificações por Telegram e uma rota Android para conexão ao servidor.

## Recursos

- modo rápido e modo detalhado para criação de karaokê;
- upload de áudio, vídeo e imagens;
- importação opcional de conteúdo do YouTube por `yt-dlp`;
...
- administrador com visibilidade e operação sobre as bibliotecas de todas as contas;
...
/data/library/history/   vídeos finais
/data/output/models/     modelos de IA
...
O build de lançamento utiliza `VERSION_NAME=7.0.0` e `VERSION_CODE=70000`. A assinatura deve ser fornecida somente pelo ambiente privado de compilação e nunca deve ser adicionada ao repositório.
